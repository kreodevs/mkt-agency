...
export function buildFramePrompt(basePrompt: string, index: number, total: number): string {
  ...
  return `${basePrompt}\n\nFrame ${index + 1} de ${total} para carrusel en redes. Mantén continuidad visual con los demás frames. Llena todo el lienzo; no dejes recuadros vacíos ni placeholders en la esquina superior izquierda (el logo real se superpone después).`;
}
