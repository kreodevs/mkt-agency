...
Campo `platform` (red social destino) también es metadata-only: editable vía `PATCH` sin nueva versión (`scheduledDate`, `visualFormat`, `visualPrompt`, `platform`, `visualTemplateId`, `visualScene`, `visualHeadline`, `visualSubline`, `visualCta`). `visualTemplateId` acepta plantillas SVG (`product-hero`, …) y presets de pipeline (`creative-scene`, `ai-art`). `visualScene` fija el arquetipo cuando el preset es escena creativa.
