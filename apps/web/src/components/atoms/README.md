...
- **Button** — variantes primarias (`default`, `outline`, `ghost`, `brand`, …) y de acción (`action`, `action-destructive`, …). Tamaños `default|sm|lg|icon|action`. Estado `disabled`: sin `opacity` global; `brand` usa fondo `--brand-muted` y texto `--brand-deep` para contraste en cards claras.
