...
- `LlmModelSelect.tsx` — autocompletar de modelos OpenRouter (chat + Image API); lista en portal con `bg-[var(--card)]` sobre modales; búsqueda por nombre/slug y valor manual con Enter. Props `taskType` prioriza modelos de imagen en `image_generation`.
