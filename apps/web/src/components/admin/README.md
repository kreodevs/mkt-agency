...
- `LlmModelSelect.tsx` — autocompletar de modelos OpenRouter (chat + Image API + Video API); lista en portal con `data-llm-model-listbox` (compatible con modales Radix); selección en `mousedown`; búsqueda por nombre/slug y valor manual con Enter. Props `taskType` prioriza modelos de imagen/video.
