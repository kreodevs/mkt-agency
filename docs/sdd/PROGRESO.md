...
- [x] [P] Implementar endpoint `GET /api/v1/company-profile` para obtener perfil
  **Nota:** Requiere `tenantId` en JWT (`TenantGuard`).
...
- [ ] [P] Implementar lógica de eliminación protegida: verificar `is_in_use` y `reference_count`
  **MDD:** §5.1 regla 9
  **Story:** US-015
...
- [ ] [P] Implementar worker para emisión de SSL vía Let's Encrypt tras verificación exitosa
  **MDD:** §7.2
  **Story:** US-016
...
- [x] [P] Configurar proyecto React con Vite, Tailwind CSS 3.4 y Kreo UI
  **Archivo:** `apps/web/package.json`, `apps/web/vite.config.ts`

- [x] [P] Configurar React Router v6 con lazy loading por módulo
  **Archivo:** `apps/web/src/router/index.tsx`

- [x] [P] Configurar TanStack Query (React Query) para estado del servidor
  **Archivo:** `apps/web/src/providers/QueryProvider.tsx`

- [x] [P] Configurar Zustand para estado global (sesión, UI state)
  **Archivo:** `apps/web/src/store/auth.ts`

- [x] [P] Implementar manejo de tokens JWT en memoria (nunca localStorage)
  **Archivo:** `apps/web/src/store/auth.ts`, `apps/web/src/services/auth.ts`

- [x] [P] Implementar refresh automático de token en cliente API
  **Archivo:** `apps/web/src/services/api.ts`

- [x] [P] Crear layout con sidebar y header (Kreo AppLayout + SidebarModern)
  **Archivo:** `apps/web/src/components/organisms/AppLayout.tsx`

- [x] [P] Implementar protección de rutas según rol (JWT guard)
  **Archivo:** `apps/web/src/guards/AuthGuard.tsx`

- [x] [P] Páginas bootstrap: `/setup`, `/login`, dashboard inicial
  **Archivo:** `apps/web/src/pages/**`
...
- [ ] [P] Crear página de configuración de dominio personalizado
  **MDD:** §2.4
  **Story:** US-016
...
### Sesión 2026-06-26 — US-001 (parcial)

- Gobernanza IA instalada vía `scripts/install-agent-governance.sh`.
- Scaffold inicial del monorepo: `package.json` (workspaces) + `apps/backend` (NestJS 10, TypeORM, PostgreSQL).
- Implementado `GET /api/v1/setup/status` en `SetupModule` con puerto `UserRepositoryPort` y adaptador TypeORM (`countSuperadmins`).
- `yarn build` OK.
- **Conflicto SDD:** `.specify/memory/constitution.md` no presente en el ZIP; se priorizó `docs/sdd/mdd.md` como MDD operativo (ver `AGENTS.md`).

### Sesión 2026-06-26 — US-001 (completo)

- `POST /api/v1/setup/init` con `SetupInitRequestDto` (class-validator) y respuesta 201 según MDD §4.
- `NoSuperadminExistsGuard` → 409 `{ error, code: "CONFLICT" }`.
- CQRS: `CreateSuperadminCommand` + `CreateSuperadminHandler` (doble chequeo de superadmin en dominio).
- `Password` value object: política MDD (8+, mayúscula, número, especial) + hash Argon2id (`memoryCost=65536`, `parallelism=4`).
- `ValidationPipe` global en `main.ts`.
- Checkpoint US-001 listo para smoke test manual con PostgreSQL.

### Sesión 2026-06-26 — US-002 (completo)

- `TenantModule` con CRUD `/api/v1/tenants` (JWT+SA).
- `TenantEntity` + `TenantRepositoryPort` / `TypeOrmTenantRepository` (list con filtros `status`, `plan`, paginación).
- CQRS: `CreateTenantCommand` (transacción tenant+owner) y `DeleteTenantCommand`.
- Guards: `JwtAuthGuard` + `SuperadminGuard` + `JwtStrategy` (HS256 vía `JWT_SECRET`; login en US-004).
- Nota SDD: regla superadmin mínimo en DELETE tenant referenciada en tasks aplica a `DeleteUserCommand` (MDD §5.1.6); delete tenant solo cascade usuarios del tenant.
- Frontend: listado `/tenants` con `DataTable` Kreo (`apps/web/src/pages/tenants/TenantListPage.tsx`).

### Sesión 2026-06-26 — US-004 + US-003 (completo)

**Orden elegido:** auth (US-004) antes de impersonación (US-003) para desbloquear JWT real en tenants.

- `AuthModule`: login/refresh/logout/jwks; RS256 (`JwtTokenService`); refresh opaco SHA-256 + rotación + detección reuso (409).
- `LoginHandler`: Argon2id, bloqueo 5×15min, eventos `security_events`.
- `SuperadminModule`: POST/DELETE `/superadmin/impersonate`, logs en `impersonation_logs`, `ImpersonationPolicy`.
- Guard global `JwtAuthGuard` + `@Public()` en setup/auth.

### Sesión 2026-06-26 — US-005 + US-006 (completo)

- `UsersModule`: `GET/PATCH /users/me` (PATCH solo `name` según contrato API); `audit_logs` en updates.
- `SecurityModule`: `GET /security-events` paginado (superadmin).

**Próximo bloque sugerido:** US-007 frontend (wizard onboarding company-profile); creación tenant en UI; US-008 sugerencias IA.

### Sesión 2026-06-26 — Listado tenants (frontend)

- `TenantListPage` en `/tenants` con Kreo `DataTable`, `StatusPill`, filtros API (`status`, `plan`).
- Servicio `services/tenants.ts` alineado a respuesta `{ items, total, page, limit }`.
- `DashboardShell` compartido para layout superadmin.

### Sesión 2026-06-26 — US-007 backend + scaffold frontend

- `CompanyProfileModule`: GET/PATCH `/company-profile`, GET/PATCH `/sections/:key`; `TenantGuard`; outbox `CompanyProfileCompletedEvent` al ≥80%.
- Perfil vacío creado en transacción al crear tenant (`CreateTenantHandler`).
- `apps/web`: Vite + React + Tailwind + Kreo (Button, InputText, Password, Card, AppLayout, etc.); páginas `/setup`, `/login`, dashboard.
- `yarn build` OK (backend + web).

### Sesión 2026-06-26 — US-007 frontend (wizard onboarding)

- `OnboardingWizardPage` en `/onboarding` (8 secciones, PATCH por paso, progreso 80%).
