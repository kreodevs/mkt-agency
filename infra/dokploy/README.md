...
### Limpiar contenidos generados (bandeja «Por aprobar»)

`SKIP_GENERATED_CONTENT_RESET=false` **no** borra nada por sí solo en cada deploy (la migración `0032` solo corre una vez).

Para vaciar contenidos al desplegar, en el servicio **`api`** (un solo deploy):
...
[entrypoint] Generated contents clear finished.
```

Luego quita `RUN_GENERATED_CONTENT_RESET` o ponla en `false` antes del siguiente deploy.

Manual en terminal `api`:
