...
| `redis-docker-entrypoint.sh` | Entrypoint del servicio `redis` en Compose: repara AOF corrupto (`redis-check-aof --fix`) o resetea persistencia si falla (`REDIS_AOF_RESET_ON_CORRUPT`, default `true`). |
